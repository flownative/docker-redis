FROM harbor.flownative.io/docker/base:bullseye
...
ENV FLOWNATIVE_LIB_PATH=/opt/flownative/lib \
    REDIS_VERSION="5:6.0.16-1+deb11u8" \
    REDIS_BASE_PATH=/opt/flownative/redis \
    PATH="/opt/flownative/redis/bin:$PATH" \
    LOG_DEBUG=false

USER root
COPY --from=harbor.flownative.io/docker/bash-library:1 /lib $FLOWNATIVE_LIB_PATH
